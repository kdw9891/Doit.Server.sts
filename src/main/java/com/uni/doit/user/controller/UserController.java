 ...
 */
@Tag(name = "user")  // Swagger와 같은 API 문서화 도구에서 이 컨트롤러의 태그를 설정하는 어노테이션
@RestController  // 이 클래스가 REST API 요청을 처리하는 컨트롤러임을 나타냄
@RequestMapping("/user")  // 이 컨트롤러의 기본 경로를 설정합니다. "/user" 경로로 들어오는 요청을 처리합니다.
public class UserController {
    ...
    public ResponseEntity<?> loginUser(@RequestBody LoginRequest loginRequest) throws IOException {
        ...
        ResponseEntity<Map<String, Object>> validationResult = requestUtils.validateDtoParams(loginRequest, "user_id", "user_name");
        ...
        return userService.loginUser(params);  // 서비스 로직 호출 및 결과 반환
    }
    ...
    public ResponseEntity<?> testloginUser(@RequestParam String user_id, @RequestParam String user_name) throws IOException {

        // RequestParam으로 전달된 파라미터들을 Map으로 변환
        Map<String, Object> params = ParamUtils.createParams("user_id", user_id, "user_name", user_name);
